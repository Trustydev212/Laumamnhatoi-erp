# 🧪 Test script cho Nhà Tôi ERP trên local (PowerShell)
...
try {
    ...
    Write-Host "⚠️  PM2 not found. Please install PM2 globally:" -ForegroundColor Yellow
...
$normalizedPath = $projectRoot.Replace([System.IO.Path]::DirectorySeparatorChar, "/")
$localEcosystem = @"
module.exports = {
  apps: [
    {
...
try {
    ...
    Write-Host "⚠️  Backend health check failed at $backendUrl" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "⚠️  Frontend health check failed at $frontendUrl" -ForegroundColor Yellow
